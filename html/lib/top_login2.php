...
if (isset($authUrl)){
	...
	echo '<a class="login" href="' . $authUrl . '">로그인</a>';
	echo '</div>';
	...
	$sql ="SELECT COUNT(g_id) as usercount FROM user WHERE g_id=$user->id";
	...
	if($user_count){ // 이미 가입자 정보가 있으면
		...
		// et 세션정보 생성하자
		include_once 'lib/g_connect.php';
		$sql = "SELECT * FROM user WHERE g_id='$user->id';";
		$result = mysql_query($sql,$db);
		$row = mysql_fetch_array($result);
		...
		$query ="INSERT INTO user(g_id, g_email, g_name) VALUES ('$id','$email','$name')";
		echo $query;
		mysql_query($query);
    }
...
echo '</div>';
	...
	{
	?>
		<a href="./album/member_info.php"><span class="h2_txt">정보수정</span></a>
